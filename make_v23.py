...
def generate_free_form_qa_pairs(chunk_id: str, chunk_text: str, model_name: str, 
    ...
    user_message_2 = (
        ...
        f"CONTEXT: <1-2 sentences establishing domain/topic context>\n"
        f"QUESTION: <the question>\n"
        f"ANSWER: You may provide brief reasoning, but your final answer MUST be enclosed in \\boxed{{}}.\n"
        f"The boxed answer should be concise (1-3 words, a number with units, or a short phrase).\n"
        f"Example formats: \\boxed{{increased expression}}, \\boxed{{2.5 μg/mL}}, \\boxed{{72 hours}}, \\boxed{{Type II error}}\n"
        f"Only the content inside \\boxed{{}} will be used for grading.\n"
    )
    ...
    # First try to extract boxed answer
    boxed_match = re.search(r'\\boxed\{([^}]+)\}', step2_output)
    if boxed_match:
        answer = boxed_match.group(1).strip()
    else:
        # Fallback to old format if no boxed answer found
